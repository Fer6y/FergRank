...
import { classifyStyle, type ScheduleContext } from '@/lib/advancedStats';
...
import CompareStatTable from '@/components/CompareStatTable';
...
function Comparison({ pa, pb }: { pa: FighterProfile; pb: FighterProfile }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ── Striking & grappling — one shared head-to-head card directly under
          the gauntlet: each fighter labelled on a side, stats mirrored across
          the centre label, each side's standouts coloured/badged vs its own
          division median (CompareStatTable). ── */}
      <CompareStatTable pa={pa} pb={pb} />
      ...
      {/* ── Strength of schedule + win probability — side by side at the bottom ── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-stretch">
        ...
        {/* Win probability */}
        {haveElo && (
        <div className="rounded-xl px-4 py-3 flex flex-col justify-center" style={cardStyle}>
      ...
      <p className="text-[10px] leading-snug px-1" style={{ color: 'var(--text-muted)' }}>
        Strength of schedule is the mean recent-opponent Elo (with the step vs career) over the 0–100 SoS score.
        The win probability is Elo adjusted for age and the style matchup (never odds). Display-only.
      </p>
  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 flex items-start gap-3 pl-2">
        {/* Rating — final (primary) + a smaller peak beside it */}
        {p.ranked?.finalRating != null && (
          <div className="text-right">
            <div className="text-[9px] tracking-widest" style={{ color: 'var(--text-muted)' }}>RATING</div>
            <div className="flex items-baseline justify-end gap-1.5 mt-0.5">
              <span className="font-mono text-base leading-none" style={{ color: 'var(--text-primary)' }}>
                {fmt1(p.ranked.finalRating)}
              </span>
              {p.eloPeak != null && (
