...
// current-form grade leads, ceiling forecast second. The per-corner markup
// lives in components/ScoutCorner.tsx, SHARED with /contender-series so the
// two surfaces render identically. Only renders on DWCS bouts.
  ...
  return (
    <div className="border-t px-3.5 py-2.5 sm:px-4 sm:py-3" style={{ borderColor: 'var(--border)' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2.5">
        {bout.scout1 && <ScoutCorner label={lastName(bout.fighter1.name)} s={bout.scout1} />}
        {bout.scout2 && <ScoutCorner label={lastName(bout.fighter2.name)} s={bout.scout2} />}
      </div>
    </div>
  );
}
...
export default function UpcomingClient({ events }: { events: UpcomingEvent[] }) {
