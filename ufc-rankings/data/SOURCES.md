...
| `regional_arrival.csv` | Built by `research/regional/arrivalRegional.ts`: each fighter's regional Elo snapshotted immediately BEFORE their UFC debut — "how good were they when they arrived", frozen so their UFC bouts can't inflate it. 1,382 fighters with 3+ prior regional bouts; percentile is vs other UFC ARRIVALS, not the whole regional pool. **Display-only**, read by `loadRegionalRatings.getArrivalIndex()` for /prospects; joined on OUR fighter id (a real id join, no namesake risk). |
| `regional_ratings_pit.csv` | Same idea at the Contender Series cutoff (`research/regional/pitRegional.ts`) — the non-leaky artifact any predictive work must use instead of the settled ratings file. |
