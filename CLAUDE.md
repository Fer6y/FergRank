...
- **Inactivity regression** — between fights (and once more up to "today"), a rating drifts toward the mean: `rating = 1500 + (rating − 1500) × 0.92^yearsOut`, after a 6-month grace. Gentle, so an injured elite ~18 months out isn't nuked, but a long-retired-then-returning name has faded.
