...
| Fighter profile `/fighter/[id]` | ✅ | **Reorganized 2026-07-03** — analytics-dashboard-led (`AdvancedAnalytics.tsx` renders 3 stacked cards): Hero → **identity/ELO/PEAK header** → **Block A** Gauntlet + trend read \| fight history (deltas + differential strips) → **Block B** strike-ratio panel \| schedule strip + pace table → **Block C** de-emphasized radar + durability + finish anatomy ("RADAR REWORK PENDING", dashed/low-opacity until the radar is fixed) → **"why this rank"** decomposition + snapshot below the dashboard. Unranked/no-charted fighters get a simpler fallback grid |
