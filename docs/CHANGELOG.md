...
- **REFUTED: betting the model's disagreement with the closing line. Measured
  ROI −15.7% over 269 bets. No scoring change; three new research scripts.**
  Prompted by a user asking for a card readout with "where would you bet for
  value" — the answer given on 2026-08-20 named four plays generated by the rule
  *back the side where the model beats the de-vigged close by ≥13pt*. That rule
  had never been tested. It loses:
  `research/backtest/disagreementSlice.ts` (1,524 odds-matched decisive bouts,
  point-in-time ratings, closing prices) → **269 bets at ≥3 priors, 92-177,
  ROI −15.7%, hit rate 34.2%**; without the prior filter 580 bets, ROI −14.4%.
  **The mechanism is the market-gap audit's bias, read backwards.** Because the
  model systematically under-states the favourite (2026-08-18), "model − market
  ≥ +13pt" resolves to *back the underdog* almost every time — and the slice
  table shows the model is simply wrong there. Where the model sits ≥25pt under
  the favourite (n=125) the favourite realizes **74.4%**, the market said
  **73.2%**, the model said **42.2%**. At 15–25pt under (n=301): 70.1% realized
  vs market 69.3% vs model 50.0%. **In every slice the market lands within
  ~1.5pt of the truth and the model is off by up to 32pt** — the disagreement
  carries no information, it IS the bias. Flat-betting the model's side returns
  −13% to −16% in every bucket. Corroborated at the model level by
  `last100.ts` at NBOUTS=400: market 68.8% acc / 0.6179 LL vs ranked+overlay
  63.5% / 0.6535, t = 2.38 against the model, and pick disagreements running
  **market right 57, model right 36**. Breadcrumb: the edge column of a card
  report is a bias readout, not a value signal — do not re-derive these bets.
- **CONFIRMED (display-grade): the finish/ITD read is real.**
  `research/backtest/finishSignal.ts` builds point-in-time finish-win and
  times-finished rates off `FightTrace` (strictly prior bouts — the settled
  career rate on a profile today contains the outcome) over 3,896 bouts with
  both corners ≥3 prior UFC fights. Base rates ITD 50.3% / KO 33.1%. All three
  candidate signals are **monotone across all four quartiles**: both-are-finishers
  39.0→62.9% (AUC 0.603), finisher-vs-chin 36.3→66.8% (AUC 0.633), and
  KO-threat → KO specifically 21.7→48.4% (AUC 0.634). A `CARD_DATE=` mode scores
  an upcoming card against the same bands from the SAME function bodies, so the
  calibration and the application cannot drift (the CompareGauntlet lesson
  applied in advance). **Deliberate limit recorded with it**: we hold no
  ITD/method prop odds, so this establishes the read is real and says nothing
  about beating a bookmaker's price — books price finish rates too, and after
  the moneyline result the prior should be that they are efficient here as well.
  Display/analysis only; feeds nothing.
- **New: `research/backtest/cardReport.ts`** — single-card model-vs-market
  scorecard (point-in-time, dedupes duplicate BFO slugs, grades a mechanical
  betting rule at closing prices). Run on UFC 330 (2026-08-15, 10 matched
  bouts): market 60% acc / 0.9016 LL, pure Elo 40% / 0.8751, ranked+overlay
  40% / 0.9378 — a chaotic card where four favourites lost, including Orolbai
  at 1.07. The withdrawn rule went 2-4 there for **+5.25u**, of which **9.00u
  came from one 9.00 longshot (Wells)**; excluding it the same six picks return
  −2.75u. Recorded as the worked example of why an n=6 card result is noise
  against a 269-bet measurement.
- **DATA BUG (pre-existing, partly cleaned): BestFightOdds lists some events
  under TWO slugs with identical lines, and the row-keyed harnesses
  double-count them.** Found while building the above — `last100.ts` printed
  Tresean Gore's bout twice inside one 400-bout sample. Cause: `refreshRecent.ts`
  merges by SLUG, so when BFO carries a second listing for the same card both
  survive. Measured across `bfo_odds.csv`: 26 bouts duplicated (26 extra rows of
  1,910), confined to `ufc-329` (14) and `ufc-330` (12). **The `ufc-330-4297`
  half was introduced by a refresh run on 2026-08-20 and has been removed** —
  verified first as pure redundancy (identical bout set, zero differences in
  date/odds/n_books vs `-4237`), leaving `bfo_odds.csv` byte-identical to the
  last ingest. The pre-existing `ufc-329` pair is left alone. Small enough not
  to move any published finding, but it inflates recent-window samples
  specifically, which is where the newest cards live. The three new scripts
  dedupe by fighter-pair + date; `last100.ts`, `marketGapAudit.ts` and the rest
  still do not. Real fix is at the merge boundary in `refreshRecent.ts` (dedupe
  on pair+date, not slug) when someone next touches the odds pipeline.
- **Pre-registered card record**: `research/picks/2026-08-22-sacramento.md` —
  the retraction, the calibrated finish projections, and a grading rule fixed
  before the event, per the skid-study pattern.
